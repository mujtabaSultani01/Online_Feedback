...
$sql=mysqli_query($conn,"select * from Company where password='$op' and email='".$_SESSION['Company_login']."'");
...
if($r==true)
{
	...
	if($np==$cp)
	{
	$sql=mysqli_query($conn,"update Company  set password='$np' where email='".$_SESSION['Company_login']."'");
